            ...
            <table class="table table-report mt-2" aria-label="customer-list">
                ...
                <tbody id="todo-list">
                    @php
                        $no = 0;
                    @endphp
                    @foreach ($productrecommend as $clist)
                        <tr class="intro-x">
                            <td>{{ ($page - 1) * 15 + ++$no }}</td>
                            <td>
                        <div class="flex">
                            <div class="w-10 h-10 image-fit zoom-in">
                                 <img class="rounded-full cursor-pointer" 
                                     src="{{ Str::startsWith($clist->productImage, ['http://','https://']) ? $clist->productImage : '/' . $clist->productImage }}"
                                     onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                     alt="Customer image"
                                     onclick="openImage('{{ Str::startsWith($clist->productImage, ['http://','https://']) ? $clist->productImage : '/' . $clist->productImage }}')" />
                            </div>
                        </div>
                    </td>
                            <td>
                            ...
                            <td>
                                <div class="font-medium whitespace-nowrap">
                                    {{ $clist->product_recommend ? 'Purchased' : 'Not Purchased' }}</div>
                            </td>
