...
@section('subhead')
    <title>{{ucfirst($professionTitle)}} Assistant</title>
@endsection

@section('subcontent')
    <div class="loader"></div>
    <h2 class="intro-y text-lg font-medium mt-10 d-inline">{{ucfirst($professionTitle)}} Assistant</h2>
            ...
            <table class="table table-report mt-2" aria-label="customer-list">
                <thead class="sticky-top">
                    <tr>
                        ...
                        <th class="whitespace-nowrap">Assistant</th>
                            ...
                            <td>
                                <div class="flex">
                                    <div class="w-10 h-10 image-fit zoom-in">
                                     ...
                                     src="{{ Str::startsWith($clist->profile, ['http://','https://']) ? $clist->profile : '/' . $clist->profile }}"
                                     onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                     alt="Customer image"
                                     onclick="openImage('{{ Str::startsWith($clist->profile, ['http://','https://']) ? $clist->profile : '/' . $clist->profile }}')" />
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="flex">
                                    <div class="w-10 h-10 image-fit zoom-in">
                                    <img class="rounded-full cursor-pointer" 
                                     src="{{ Str::startsWith($clist->profileImage, ['http://','https://']) ? $clist->profileImage : '/' . $clist->profileImage }}"
                                     onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                     alt="Customer image"
                                     onclick="openImage('{{ Str::startsWith($clist->profileImage, ['http://','https://']) ? $clist->profileImage : '/' . $clist->profileImage }}')" />
                                    </div>
                                </div>
                            </td>
            ...
            <nav class="w-full sm:w-auto sm:mr-auto">
                <ul class="pagination" id="pagination">
                    <li class="page-item {{ $page == 1 ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ route('productRecommend', ['page' => $page - 1]) }}">
                    ...
                    @for ($i = 0; $i < $totalPages; $i++)
                        <li class="page-item {{ $page == $i + 1 ? 'active' : '' }} ">
                            <a class="page-link"
                                href="{{ route('productRecommend', ['page' => $i + 1]) }}">{{ $i + 1 }}</a>
                        </li>
                    @endfor
