...
@if ($aiAstrologers->isEmpty())
<a class="btn btn-primary shadow-md mr-2 mt-10 d-inline addbtn" href="{{route('create.ai.chat.bot')}}">Add AI Chat Bot</a>
@endif
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center">
        ...
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <form action="{{ route('ai.chat.bot') }}" method="GET">
                @csrf
                <div class="w-56 relative text-slate-500" style="display:inline-block">
                    ...
                    <a href="{{ route('ai.chat.bot') }}" class="text-slate-500" onclick="clearSearch()">
            ...
            <tr class="intro-x">
                        ...
                        <div class="flex">
                            <div class="w-10 h-10 image-fit zoom-in">
                                <img class="rounded-full cursor-pointer" 
                                     src="{{ Str::startsWith($astro->image, ['http://','https://']) ? $astro->image : '/' . $astro->image }}"
                                     onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                     alt="Customer image"
                                     onclick="openImage('{{ Str::startsWith($astro->image, ['http://','https://']) ? $astro->image : '/' . $astro->image }}')" />
                            </div>
                        </div>
                    </td>
                ...
                <td class="table-report__action w-56">
                    ...
                    <div class="flex justify-center items-center">
                        ...
                        <a class="flex items-center mr-3" href="{{ route('edit.ai.chat.bot', ['slug' => $astro->slug]) }}">
                            <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit
                        </a>

                        {{-- <a class="flex items-center mr-3 text-danger" href="javascript:void(0);" onclick="deleteMasterAiChatBot({{@$astro->id}})">
    ...
    <nav class="w-full sm:w-auto sm:mr-auto">
        <ul class="pagination">
            <li class="page-item {{ $page == 1 ? 'disabled' : '' }}">
                <a class="page-link" href="{{ route('ai.chat.bot', ['page' => $page - 1, 'searchString' => $searchString]) }}">
            ...
            <li class="page-item">
                <a class="page-link"
                href="{{ route('ai.chat.bot', ['page' => 1, 'searchString' => $searchString]) }}">1</a>
            </li>
            @if ($startPage > 2)
            ...
            <li class="page-item {{ $page == $i ? 'active' : '' }}">
                <a class="page-link"
                href="{{ route('ai.chat.bot', ['page' => $i, 'searchString' => $searchString]) }}">{{ $i }}</a>
            </li>
            @endfor
            ...
            <li class="page-item">
                <a class="page-link"
                href="{{ route('ai.chat.bot', ['page' => $totalPages, 'searchString' => $searchString]) }}">{{ $totalPages }}</a>
            </li>
            @endif
...
<script>
    function deleteMasterAiChatBot(id) {
        ...
        .then((willDelete) => {
            if (willDelete) {
                axios.delete("{{ route('delete.ai.chat.bot', '') }}" + '/' + id)
                .then(response => {
                    swal({
